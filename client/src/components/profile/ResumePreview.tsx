...
function getCleanedResumeData(originalData: any) {
  ...
  if (!data.personalInfo) {
    ...
    if (!data.personalInfo.name) data.personalInfo.name = "Barathi";
    if (!data.personalInfo.email) data.personalInfo.email = "balaabirami1997@gmail.com";
    if (!data.personalInfo.phone) data.personalInfo.phone = "6379607758";
    if (!data.personalInfo.location) data.personalInfo.location = "Chennai, Tamil Nadu";
  }

  // Dummy Bio Objective if empty or generic
  if (!data.personalInfo.bio || getCleanBio(data.personalInfo.bio) === "") {
    data.personalInfo.bio = "Dedicated Professional combining analytical thinking with hands-on expertise to solve complex problems and deliver exceptional value. Brings a proactive approach to team collaboration and success.";
  }
  ...
  if (hasSkills === 0) {
    data.skills = {
      technical: ["react", "html", "skill", "leaders", "tamil", "enmgilsh", "malayalam"],
      soft: ["Team Collaboration", "Problem Solving", "Time Management"],
  ...
  // Dummy Performance Ratings if missing
  if (!data.performance) {
    data.performance = {
      avgTechnical: 0,
      avgCommunication: 0,
      highlights: ["Technical Core", "Problem Solving", "Team Alignment"]
    };
  }
...
export default function ResumePreview({ isOpen, onClose }: ResumePreviewProps) {
  ...
  const { data: rawResumeData, isLoading, isError } = useQuery({
    queryKey: ["userResume", userId],
    queryFn: async () => {
  ...
  const resumeData = getCleanedResumeData(rawResumeData);
      ...
      <div className="flex-1 overflow-y-auto bg-slate-200/50 relative flex justify-center items-start p-8 lg:p-12 print:p-0 print:overflow-visible custom-scrollbar">
         ...
         {isLoading ? (
            ...
            <div
              ref={resumeRef}
              id="resume-content"
              className="bg-white w-full max-w-[210mm] min-h-[297mm] flex flex-col shadow-2xl p-[10mm] rounded-sm overflow-hidden h-fit print:shadow-none print:w-full print:max-w-none print:min-h-screen print:absolute print:inset-0 print:z-[9999] print:rounded-none transition-all duration-500 mx-auto relative z-10"
            >
                  {isSidebarLayout ? (
                    <div className="grid grid-cols-12 gap-0 border border-slate-200 rounded-xl overflow-hidden flex-1">
                      <aside className={`col-span-4 p-5 ${templateStyle.chipBg} h-full`}>
